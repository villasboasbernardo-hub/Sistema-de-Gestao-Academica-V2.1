...
describe("Fronteira 1 — lib/dominio/ é puro (Princípio II, risco R-10)", () => {
  it("BARRA import de cliente de banco dentro de lib/dominio/", async () => {
    ...
    expect(violacao, "a fronteira 1 não acusou o import proibido — regra desligada?").toBeDefined();
    expect(violacao?.severity).toBe(2); // erro, não aviso: aviso não barra merge
    expect(violacao?.message).toContain("puro");
  });

  it("DEIXA PASSAR uma função pura equivalente — a regra não é um bloqueio cego", async () => {
    const eslint = new ESLint({ cwd: RAIZ });
...
describe("Fronteira 2 — a service_role nunca chega ao navegador (risco R-09)", () => {
  it("BARRA import de @/lib/supabase/admin em componente de navegador", async () => {
    const mensagens = await lintarComo("cliente-importa-admin.tsx", "app/_fixture-fronteira-2.tsx");

    const violacao = mensagens.find((m) => m.ruleId === "no-restricted-imports");

    expect(violacao, "a fronteira 2 não acusou o import de admin — regra desligada?").toBeDefined();
    expect(violacao?.severity).toBe(2);
    expect(violacao?.message).toContain("service_role");
  });

  it("DEIXA PASSAR o próprio admin.ts — ele é a exceção nomeada", async () => {
    const eslint = new ESLint({ cwd: RAIZ });
...
describe("A isenção dos fixtures continua estreita (T020, achado A1)", () => {
  it("isenta APENAS tests/unidade/lint/fixtures/ — alargar desliga a fronteira em silêncio", async () => {
  ...
  it("os fixtures continuam existindo — sem eles, os testes acima passam por vacuidade", async () => {
    await expect(
