---
title: "CIAARA-11 v2.1 — Constitution"
author: "Arquiteto Chefe · CIAARA-11 (Divisão de Administração Acadêmica)"
date: "26/08/2026"
version: "2.1"
---
...
> **O que este documento é.** O conjunto de princípios que prevalece sobre qualquer plano, spec,
> tarefa ou sugestão de agente. É o único artefato que o agente de código carrega em **todos** os
> comandos do Spec Kit — e por isso é o lugar onde moram as regras que não podem ser esquecidas no
> décimo quarto épico.
>
> **O que este documento não é.** Não é requisito, não é arquitetura e não é tutorial. Requisito
> mora na Fase 1 (`Fase 1 - Requisitos/00` a `07`); arquitetura mora na Fase 2
> (`Fase 2 - Arquitetura/20` a `25`); procedimento operacional mora no documento 10
> (`Fase 1 - Requisitos/10-Plano-de-Execucao-Vibe-Coding.md`) e no documento 41
> (`Vibe Coding/41-Guia-de-Vibe-Coding.md`).

## Nota de migração (v2.1)

Esta constitution **sucede** a `CIAARA-11-v2/.specify/memory/constitution.md` (versão 1.2.0,
ratificada em 11/08/2026, emendada em 14/08/2026). Ela **não a apaga**.

A v2.0 é um sistema **em produção**: Google Apps Script (V8) + Google Sheets + Vanilla JS/Bootstrap 5,
com 39 specs Spec Kit executadas e base migrada e saneada. A v2.1 é a **migração dessa mesma
plataforma** para Next.js + Supabase (PostgreSQL), mantendo intactos requisitos, regras de negócio e
vocabulário institucional.

**Regra de ouro da versão:** a v2.1 **não reinventa o domínio**. Ela reimplanta o mesmo domínio
noutra plataforma. Toda regra `RN-`, requisito `RF-` e restrição `RNF-` tem destino explícito —
*preservada*, *preservada com nova implementação*, *substituída pela plataforma* ou *aposentada* —
e o destino é sempre nomeado, nunca subentendido.

Dos nove princípios da v1.2.0: **oito são preservados** (um deles reforçado, outro estendido), **um
é reescrito** (o III), e **dois são acrescentados** (X e XI). Nenhum foi deletado. A trilha da v2.0
até a v2.1 precisa continuar legível por quem chegar depois de nós.
...
> sem discussão. Está registrado em `docs/sql-referencia/05_rls_policies.sql` no ponto exato, no documento 22 §6.2 e
> aqui.
