...
| **Épico 4 — fatia (c): casca e estado na URL** | 🟨 **IMPLEMENTADA em 11/09/2026. Medido**: `pnpm verificar:tudo` sai **0** — **318** de unidade · **102** pgTAP · **104** RLS · **132** ponta a ponta (2 pulados). O contrato de parâmetros virou **tipo**: parâmetro fora dele **não compila**, provado por defeito deliberado. A casca (`components/casca/`) é **servidor**, com três folhas de cliente declaradas e contadas por teste. A tela **`/inicio`** entrega o panorama por turma com o recorte na URL, e a raiz deixou de ser um beco. O guia `docs/guias/estado-na-url.md` é o que as telas dos Épicos 5 a 9 leem antes de errar. ⚠️ **Falta mesclar o PR e quatro decisões de Bernardo** — ver a tabela de pendências |
...
| **MENU-1** | **Validar o menu lateral contra a v2.0 em produção** — a ordem das entradas, o rótulo de "Disciplinas", e se Administração é entrada única ou grupo. O rascunho, as três perguntas e a tabela de registro estão em `specs/008-shell-e-estado-na-url/contracts/casca.md` | Fecha o `FR-017.1` e o `CHK017`. **Sem isso o `FR-017` não é verificável** |
| **MENU-2** | As entradas **sem tela** aparecem no menu marcadas *"em breve"*. A alternativa é mostrar só o que existe — e aí o menu cresce a cada épico, contra o `RF-NAV-02`, que é **[PRESERVADO]** | Nenhum épico; é aparência da navegação |
| **PR-4c** | **Mesclar o PR #9** (correção do redirecionamento aberto, já verde) e o PR da fatia (c) | Fecha o Épico 4 |
