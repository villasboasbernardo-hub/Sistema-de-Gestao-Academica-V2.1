...
| Épicos 5 a 13 | ⬜ Pendentes, **exceto a fatia (c) do Épico 5, acima, e a fatia (a) — spec `009-cursos-e-turmas` —, que está PLANEJADA em 17/09/2026**: spec, plano, pesquisa, modelo, contratos, quickstart e `tasks.md` com 212 tarefas em dois PRs (110 + 102), nada commitado ainda. **A dívida de estilo está paga**: as cinco telas do Épico 3 ganharam o vocabulário visual na fatia (c), e o repositório inteiro mede **zero violações** da regra de cor em 91 arquivos. **Entra no Épico 3:** a Server Action de convite (primeiro consumidor real de `lib/supabase/admin.ts`) e `NEXT_PUBLIC_URL_APLICACAO`, deixada fora do Épico 0 por decisão de 07/09 |
