...
| **Épico 5 — fatia (c): cadastro de instrutores** | 🟨 **IMPLEMENTADA em 15/09/2026, PR #16 aberto e SEM merge.** Spec `006-cadastro-de-instrutores`. `pnpm verificar:tudo` sai **0** — **513** de unidade · **149** pgTAP (nenhum `todo`) · **131** RLS e ambiente · **151 (2 pulados)** ponta a ponta. Listagem em antiguidade pelo banco, filtros da v2.0 na URL, 3 indicadores e 9 gráficos (4 barras, 5 pizzas); ficha com cadastro, edição, painel de disciplinas (`VIN-NNNNNN`), desativação que preserva o passado e **alertas que não bloqueiam** — carga semanal **por semana ISO**, somando só as atribuições cuja janela cobre a semana (somar o ano é proibido), e docência há mais de um ano sem capacitação. A escrita de CPF, RG, telefone e endereço ficou com os três perfis que os leem, **por coluna, no banco**. **Sete migrations.** 🛑 **Antes do merge, as sete precisam ser aplicadas no Supabase remoto** — o mesmo projeto serve Preview e Production (exceção do `FR-016.1`), e mesclar sem aplicar faz Production pedir colunas que não existem. **A T087 (aplicar) é de Bernardo e está aberta.** ⏸️ **Pararam depois de conferência**: legenda clicável (não está no código da v2.0, T119) e ficha A4 (falta o selo "Marinha do Brasil — Hidrografia e Navegação", T111). **Pedem decisão**: 6 itens de `specs/006-cadastro-de-instrutores/checklists/fechamento.md` (CHK004 e CHK005 decididos por Bernardo em 15/09/2026 — o CHK005 mudou o alerta para todas as semanas ISO do ano corrente, inclusive a janela vinda do ano anterior), e as T002/T003 |
