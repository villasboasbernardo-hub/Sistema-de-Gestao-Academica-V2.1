-- =====================================================================================
-- M3 — Fatos de execucao, no grao de Unidade de Ensino
-- Epico 1 · specs/002-schema-rls-permissoes
-- FR-012 · FR-014 · FR-015 · FR-016 · FR-020 · FR-027 · FR-061
-- -------------------------------------------------------------------------------------
-- O QUE  : as cinco tabelas que registram O QUE ACONTECEU (ou o que se planeja), por
--          oposicao as de M2, que dizem o que EXISTE.
-- PARA QUE: sao a base de calculo de CHD, CHT, conformidade de tetos, DSA, cronograma e
--          carga horaria docente. Cada linha aqui e historia — e historia neste sistema
--          nao se apaga nem se reescreve.
-- -------------------------------------------------------------------------------------
-- ESTA E A MIGRATION QUE DIVERGE DO REFERENCIA. Duas revisoes, ambas obrigatorias:
--
--   1. `registros_aula` NASCE NO GRAO DE UNIDADE DE ENSINO. O referencia
--      (`docs/sql-referencia/02_tabelas_fato.sql`) traz `disciplina_id NOT NULL` — o grao
--      anterior a decisao UE-1 — e o proprio arquivo avisa disso num comentario. O BRIEF
--      §2.2 e explicito: "o script de referencia NAO e a autoridade neste ponto".
--      Aqui o fato aponta para `unidades_ensino` e NAO guarda `disciplina_id`.
--
--   2. `registros_aula` e `avaliacoes` ganham as chaves compostas que tornam RN-MAT-01
--      declarativa (FR-061). As duas cadeias sao DIFERENTES: o registro de aula cruza
--      turma x unidade de ensino; a avaliacao cruza turma x disciplina.
--
-- Pre-requisito: M1 e M2 aplicadas.
-- -------------------------------------------------------------------------------------
-- REVERSAO (FR-056): `drop table` na ordem inversa —
--   planejamento_anual, atividades_nao_letivas, avaliacoes, registros_aula,
--   avaliacoes_planejadas. Segura enquanto nao houver carga.
-- =====================================================================================
...
create table public.registros_aula (
  ...
  -- =================================================================================
  -- O GRAO. Aqui, e so aqui, esta a mudanca de fundo da decisao UE-1 rota (b).
  -- O referencia traz `disciplina_id uuid not null references disciplinas(id)`. NAO
  -- copiamos: o fato de execucao nasce no grao de UNIDADE DE ENSINO, e a disciplina e
  -- alcancada POR ELA. Guardar `disciplina_id` aqui em paralelo criaria a segunda fonte
  -- de verdade que a rota (b) existe para eliminar — bastaria um lancamento em que a
  -- disciplina da coluna divergisse da disciplina da UE para ninguem saber qual vale.
  -- Obrigatorio de propósito: a tabela nasce vazia e a carga e do Epico 2, entao relaxar
  -- depois custa uma linha; nascer obrigatorio forca Q1.b a ser respondida ANTES da
  -- carga, em vez de resolvida em silencio por um NULL que ninguem notou (research §2).
  -- =================================================================================
  unidade_ensino_id       uuid not null references public.unidades_ensino(id)  on delete restrict,

  -- Componente da cadeia de RN-MAT-01. Nao e dado duplicado: as duas chaves compostas
  -- abaixo obrigam a concordar com a turma E com a unidade de ensino.
  curso_id                uuid not null references public.cursos(id)           on delete restrict,
  ...
  -- =================================================================================
  -- RN-MAT-01, DECLARATIVA (FR-061 · research.md §3 · Risco: Alto).
  -- "Uma aula so pode ser lancada para uma disciplina que pertenca ao mesmo curso da
  -- turma selecionada." Com o grao de UE, provar isso exigiria percorrer
  -- unidade_ensino -> disciplina -> curso e turma -> curso e comparar.
  -- As duas chaves compostas abaixo tornam a comparacao desnecessaria: um lancamento
  -- cuja turma e cuja unidade pertencam a cursos diferentes NAO TEM `curso_id` que
  -- satisfaca as duas ao mesmo tempo. A regra passa a ser impossivel de violar, sem
  -- gatilho e sem codigo de aplicacao — que era como a v2.0 a mantinha, e por isso ela
  -- e de risco alto: bastava esquecer a chamada num ponto de lancamento.
  -- Conferido contra o PostgreSQL 17 na tarefa T004; provado por T040.
  -- =================================================================================
  constraint reg_aula_turma_do_curso
    foreign key (turma_id, curso_id)
    references public.turmas (id, curso_id) on delete restrict,
  constraint reg_aula_ue_do_curso
    foreign key (unidade_ensino_id, curso_id)
    references public.unidades_ensino (id, curso_id) on delete restrict
);

comment on table  public.registros_aula is
  'Diário de execução letiva: aulas e atividades extraclasse (1.566 linhas a migrar). '
  'GRÃO DE UNIDADE DE ENSINO desde a decisão UE-1 rota (b) — a disciplina é alcançada pela '
  'UE, nunca guardada aqui em paralelo. AMBAS as categorias exigem unidade vinculada: é essa '
  'a fronteira com `atividades_nao_letivas`, onde não há disciplina. '
  'v2.0: `Registro_Aulas_E_Atividades` §5.5; BRIEF §2.2.';
comment on column public.registros_aula.unidade_ensino_id is
  'O GRÃO do fato. Substitui o `disciplina_id` do desenho anterior (UE-1 rota (b)). A '
  'disciplina e o curso são alcançados por aqui; a CH executada da disciplina é o agregado '
  'destas linhas, exposto por VIEW — nunca uma segunda coluna gravada.';
comment on column public.registros_aula.curso_id is
  'Componente das chaves compostas que tornam RN-MAT-01 declarativa (FR-061). Não é dado '
  'duplicado: `reg_aula_turma_do_curso` e `reg_aula_ue_do_curso` impedem que divirja.';
...
-- O referencia indexava (disciplina_id, data). Com o grao de UE essa coluna nao existe:
-- a CHD da disciplina passa a ser alcancada PELA UNIDADE DE ENSINO. Os dois indices
-- abaixo cobrem os caminhos de leitura reais — por unidade e por curso.
create index idx_reg_aula_unidade      on public.registros_aula (unidade_ensino_id, data);
create index idx_reg_aula_curso        on public.registros_aula (curso_id, data);
...
create table public.avaliacoes (
  ...
  -- Componente da cadeia de RN-MAT-01 desta tabela — que NAO e a de `registros_aula`:
  -- a avaliacao referencia DISCIPLINA, nao unidade de ensino (FR-061, research §3).
  curso_id                  uuid not null references public.cursos(id)      on delete restrict,
  ...
  -- RN-MAT-01 para avaliacao: a disciplina avaliada pertence ao curso da turma (FR-061).
  constraint aval_turma_do_curso
    foreign key (turma_id, curso_id)
    references public.turmas (id, curso_id) on delete restrict,
  constraint aval_disciplina_do_curso
...
-- =====================================================================================
-- FIM DE M3. Proxima: M4 — configuracao, calendario e matriz de permissoes.
-- =====================================================================================
