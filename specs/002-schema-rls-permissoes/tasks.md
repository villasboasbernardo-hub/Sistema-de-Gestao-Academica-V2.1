...
- [X] T091 **BLOQUEADO — não por esta fatia.** Abrir o PR exige um remote, e a raiz do git é `SIS11`, não `Versao_2.1_NextJS`: um push publicaria a **base viva** (`Banco de dados CIAARA-11.xlsx`), o Ofício do PROENS, o código da v1.0/v2.0 e todo o histórico. O desbloqueio é o **FR-021 da spec 001** (replantar a v2.1 em repositório próprio), que é do Épico 0 e move a cópia de trabalho no disco. **O corpo do PR está pronto** em [PULL_REQUEST.md](./PULL_REQUEST.md), com os seis planos de reversão e a lista de conferência. Abrir o PR com o template inteiro preenchido, **os seis planos de reversão** e a lista de conferência de [quickstart.md](./quickstart.md) §10. Commits no padrão `db(<identificador>): <resumo no imperativo>` **Encerrada em 07/09/2026 SEM PR, por decisão registrada.** O bloqueio original caducou — há remote, `main` sincronizada, CI verde e proteção aplicada. Mas o código do Épico 1 **entrou na `main` em 30/08/2026**, antes de existirem CI e portão. Abrir PR retroativo para código já mesclado **não provaria nada** e seria encenação de processo (Princípio VI: prova, não declaração). O que vale é que as seis migrations, as 80 asserções pgTAP e os 19 testes de RLS **rodam no bloco `banco` do CI a cada push**, verde — verificação contínua que um PR de uma vez não daria.
