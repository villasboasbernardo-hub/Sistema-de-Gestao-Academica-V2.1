...
| **A-8** | **O catálogo de UE vai para um repositório público.** As 572 UEs, 2.446 subunidades e a ementa de 134 disciplinas passam a ser legíveis por qualquer pessoa quando o push acontecer. Os currículos são anexos de Ofício da DEnsM, não são credencial — mas é **conteúdo programático da MB em grão bem mais fino** do que a suíte documental já exposta | `scripts/etl/dados/unidades_ensino.{csv,json}`; decisão de repositório público de 26/08/2026 | **Confirmar com Bernardo.** Se a exposição não for desejada, o caminho é `.gitignore` no diretório de dados + carga a partir da pasta local dos currículos. Fica registrado porque ninguém deve descobrir depois |
