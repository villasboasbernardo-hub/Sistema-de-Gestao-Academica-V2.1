...
- [ ] T052 [US5] Confirmar por `vercel env ls` que o escopo **Production** está **sem** variáveis de Supabase — a ausência **é** a garantia do FR-022, porque não há produção nesta fatia (FR-016.1, invariante A-3) ⚠️ **REGREDIU em 07/09/2026.** Estava provada em 03/09 (`No Environment Variables found` em Production). Em 07/09 foi cadastrada `SUPABASE_SERVICE_ROLE_KEY` nos escopos **Production e Preview**. Com isso: (a) o escopo Production deixa de estar vazio, contra o `contracts/variaveis-ambiente.md`, que lhe atribui **"Nenhuma"**; (b) o **SC-007** ("zero ambiente de produção da v2.1") e o **FR-016.1** deixam de valer; (c) o **V-9** do quickstart, que confere `vercel env ls production` esperando ausência de variável de Supabase, passa a reprovar. O contrato atribui à `service_role` os escopos **`local · CI`** — não preview, não production. **Não revertido: é ambiente do Bernardo e a decisão é dele.** Precisa de decisão explícita: emendar o contrato e o FR-016.1, ou remover das duas.
