...
- [x] CHK001 ⛔ O verbo "preservado" do FR-021 está definido? `git subtree split` mantém mensagem, autor e data mas **gera SHA novo** — o requisito não diz se isso satisfaz "preservar" [Clareza, Spec §FR-021, plan.md §Reavaliação item 1] — ✅ **Resolvido 06/09** — o README define o que "preservar" significa e mostra que o SHA muda.
- [x] CHK002 ⛔ O FR-021 fala em "**o** commit de 26/08/2026", no singular; o FR-021.2 nomeia **dois** (`d19ab10`, `d31bd56`) e a pesquisa R-1 produziu **dois** commits replantados. Quantos o replantio precisa carregar? [Conflito, Spec §FR-021 × §FR-021.2 × research R-1] — ✅ **Resolvido 06/09** — são **dois** commits, nomeados no README com a correspondência.
- [x] CHK003 A exigência de registrar a **correspondência de SHA** (antigo → novo) existe como tarefa (T037), mas não como requisito. É obrigação ou escolha do plano? [Rastreabilidade, Lacuna] — ✅ **Resolvido 06/09** — a correspondência está no README (T037).
...
- [x] CHK011 ⛔ **Nenhum dos três contextos executa a suíte de ponta a ponta** — verificado em `ci-contextos.md`: `qualidade` roda `vitest run tests/unidade`, `banco` roda pgTAP e RLS, `build` roda `next build`. Ainda assim o SC-003 exige que um defeito **de ponta a ponta** bloqueie o merge. O contrato do CI e o critério de sucesso não podem estar os dois certos [Conflito, contracts/ci-contextos.md × Spec §SC-003] — ✅ **Resolvido 03/09** — a ponta a ponta entrou no contexto `build` e roda verde no CI.
- [x] CHK012 ⛔ O FR-013 define `verificar:tudo` como "a sequência completa do CI, **incluindo o de ponta a ponta**" — e o `package.json` o implementa assim. Se o CI não roda ponta a ponta, o SC-005 (vereditos coincidem) é insatisfazível por construção nesse caso [Conflito, Spec §FR-013 × §SC-005] — ✅ **Resolvido 03/09** — com a ponta a ponta no CI, o `verificar:tudo` volta a poder coincidir (SC-005).
- [x] CHK013 ⛔ O comando de proteção declara `enforce_admins=false`. O FR-014 exige "**bloquear** o merge" e o SC-003 exige que "em nenhum dos quatro casos o merge fica disponível". Para o administrador — que é quem opera — o merge continua disponível. O requisito quer bloqueio para todos, ou portão que o responsável pode atravessar conscientemente? [Conflito, contracts/ci-contextos.md × Spec §FR-014, §SC-003] — ✅ **Resolvido 07/09** — `enforce_admins=true`. O portão barra o administrador; o SC-003 passa a ser cumprível.
- [x] CHK014 ⛔ A proteção exige `required_approving_review_count=1` num projeto de **um único operador**, e a plataforma não permite aprovar o próprio PR. Nenhum requisito diz quem aprova, nem prevê revisor único [Lacuna, Spec §FR-014.1] — ✅ **Resolvido 03/09** — `required_pull_request_reviews=null`, decisão de Bernardo: operador único não aprova o próprio PR.
...
- [x] CHK017 O FR-010 exige reprovar contrato de dados divergente, mas nenhum requisito exige que `lib/tipos/database.ts` esteja **rastreado pelo git** — sem isso o portão passa por vacuidade. Já registrado como T064; falta o requisito correspondente [Lacuna, Spec §FR-010] — ✅ **Resolvido 06/09** — `lib/tipos/database.ts` está rastreado; o portão tem o que comparar (T064).
...
- [x] CHK022 O FR-002.2 manda o procedimento estar "onde quem opera vá encontrá-lo", sem nomear o documento [Clareza, Spec §FR-002.2] — ✅ **Resolvido 06/09** — o procedimento vive no README, seção *Se um segredo vazar*.
- [x] CHK023 O **conteúdo** do procedimento de rotação não é especificado: quais chaves, em quais lugares (Supabase, Vercel, CI, `.env.local`) e em que ordem. O requisito exige o procedimento escrito sem dizer o que ele contém [Completude, Spec §FR-002.2] — ✅ **Resolvido 06/09** — o README diz quais chaves, onde e em que ordem.
