...
- [X] T002 [P] 👤 Emendar o `SC-010` em `specs/006-cadastro-de-instrutores/spec.md` para comparar colunas com `SELECT` contra a **união** de `UPDATE` e `INSERT` (`FR-032`, `SC-010`, research R-4) ⚠️ é emenda de redação do critério, não de regra; aplicar só com a confirmação de Bernardo registrada na própria linha ✅ **Aprovada por Bernardo em 15/09/2026 — é só redação** *(decisão de Bernardo Villas Boas, 15/09/2026)*: `SC-010` emendado para comparar `SELECT` com a união de `UPDATE` e `INSERT`.
- [X] T003 [P] 👤 Emendar a *Assumption 1* em `specs/006-cadastro-de-instrutores/spec.md` com o número real de migrations desta fatia — **três**, pela Fase 2, ou **quatro** se a T014 sair depois da T013 mesclada — (`FR-032`, `FR-014`, `FR-005`, `FR-007`, research R-7, D-3, D-4) ⚠️ mesma regra da T002 ✅ **Aprovada por Bernardo em 15/09/2026 — é só redação** *(decisão de Bernardo Villas Boas, 15/09/2026)*: *Assumption 1* emendada com as **nove** migrations da fatia, nomeadas.
...
- [X] T116 [US4] Acrescentar o aviso `sem-data-docencia` a `AVISOS_INICIAIS` em `lib/dominio/avisos-cadastro-instrutor.ts`, com teste, e a coluna `data_inicio_docencia_ciaara` à consulta da listagem (`FR-017`, `FR-027`, T053) ✅ O aviso cobra quem está sem a data **e** sem capacitação — é para esses que o alerta precisaria da data; o recorte, registrado primeiro como leitura aplicada, virou **decisão** em 15/09/2026 (CHK004, T123). Base real: 176 de 177 sem a data, **148** também sem capacitação.
...
- [X] T121 Atualizar `paridade.md`, `quickstart.md` e a seção *Estado atual e onde retomar* do `CLAUDE.md` (T088) ✅ `paridade.md` com a tabela das seis pendências e como fecharam; `quickstart.md` com o passo 4 da semanal por semana ISO, a contagem de 4 barras e 5 pizzas e os itens 13 a 15 do passo 5; `CLAUDE.md` com a linha da fatia e a dos achados.
- [X] T122 Recarregar a base real do ETL no banco local e recriar a conta de Bernardo pelo fluxo de convite, depois do PR aberto ✅ `pnpm db:reset` e ETL local com reconciliação APROVADA: 177 instrutores, 798 vínculos, nenhum resíduo de teste, próximo código **178**. Conta `USR-02` (admin) recriada pelo convite interceptado no Mailpit, com senha só local, e login conferido do zero na listagem com 177 ativos.

## Fase 8.3 — Respostas de Bernardo ao checklist de fechamento, 15/09/2026

**Origem**: Bernardo conferiu a fatia no ambiente local, aprovou o resultado e respondeu os itens
abertos de `checklists/fechamento.md` *(decisão de Bernardo Villas Boas, 15/09/2026)*. Tudo entra no PR #16, antes do merge.

- [X] T123 [US4] Registrar o CHK004 como decisão no `FR-017`, no contrato de carga horária, no checklist, na paridade e no cabeçalho de `lib/dominio/avisos-cadastro-instrutor.ts`: o aviso de data não informada cobra só quem também está sem capacitação (`FR-017`, `FR-027`) ✅ A regra do aviso não mudou; mudou o estatuto — de leitura a confirmar para decisão.
- [X] T124 [US4] Implementar o CHK005: o alerta de faixa avalia todas as semanas ISO do ano corrente cobertas por atribuição ativa, passadas inclusive, e a parte do ano corrente de janela vinda do ano anterior (`FR-016`, `RN-2027-06`) ✅ `limitesDoAnoIso` e `semanasDoAno` em `lib/dominio/carga-semanal.ts`, com 4 casos novos (limites do ano ISO de 2026 e 2027, janela vinda de 2025, janela que avança para 2027, semana passada); a ficha passou a pedir as atribuições com início no ano **ou** janela que toca o ano ISO, e a lista da seção de carga continua só com as de início no ano; `vw_instrutor_carga_anual` ganhou linha em todo ano que a janela toca, porque a faixa sai dela e sem linha o alerta calava (migration `20260915084854`, ainda não aplicada em lugar nenhum além do local); pgTAP `094` com a asserção 16; ponta a ponta com uma janela de 01/12 do ano anterior a 31/01 do corrente. **Conferido por defeito deliberado nos dois lados**: sem a linha dos anos tocados, a asserção 16 reprovou; com a ficha de volta ao recorte por início, o caso de ponta a ponta reprovou.

## Fase 8.4 — O restante do checklist, o quadro recolhível e a exclusão sem histórico, 15/09/2026

**Origem**: o restante das respostas de Bernardo ao `checklists/fechamento.md` e duas mudanças novas,
no mesmo PR #16, antes do merge *(decisão de Bernardo Villas Boas, 15/09/2026)*.

- [X] T125 [US2] CHK008 e CHK012 — `RN-INST-03` com cinco obrigatórios e especialidade delimitada a militar: `lib/dominio/militar-ou-civil.ts` (civil é `SC` ou `SCNS`, pelo `FR-002`), `esquemaDeCriacaoDeInstrutor` recusando militar novo sem especialidade, gatilho `trg_instrutores_especialidade_de_militar_novo` (migration `20260915140000`), aviso de obrigatório pendente cobrando especialidade só de militar (`FR-005`, `FR-027`, `RN-INST-03`) ✅ Recusa só em **cadastro novo** e só em linha sem `origem_migracao_v1` — decisão de Bernardo pedida e respondida nesta rodada: os 15 militares da base real continuam salvando a ficha. Vitest (domínio, esquema e aviso), pgTAP `098` (militar novo recusado; civil, migrado e ficha existente passam) e ponta a ponta (mensagem própria, nada gravado).
- [X] T126 [US4] CHK019 — ficha de instrutor inativo sem alerta normativo (`FR-016`, `FR-017`, `FR-018`) ✅ `alertasExibidos` só com instrutor ativo; ponta a ponta desativa o que alertava e confere zero alertas.
- [X] T127 CHK022 — registrar a medição de desempenho nas *Assumptions* da spec, sem criar requisito ✅ Item 6 das *Assumptions*.
- [ ] T128 ⏸️ CHK020 e CHK021 — legenda clicável e ficha A4 — **pendentes por falta de material**, nada implementado (`FR-026.6`, `FR-031`; ver T111 e T119)
- [X] T129 [US6] Quadro de avisos recolhível: nasce recolhido, no topo acima dos filtros, com contagem por tipo e total à vista, tipo zerado fora da linha, lista por clique com estado fora da URL (`FR-027` e `RNF-USA-04` emendados) ✅ `QuadroDeAvisos.tsx` (servidor) e a folha `AvisosRecolhiveis.tsx`; ponta a ponta confere posição, recolhido, soma do total, abrir sem mexer na URL e recorte sem aviso.
- [X] T130 [US5] Exclusão permanente de instrutor sem histórico — a exceção única à regra 4, autorizada nominalmente (`FR-008.1`) ✅ Migration `20260915140100`: `app.impedimentos_de_exclusao_do_instrutor` e `app.excluir_instrutor`, SECURITY DEFINER com porteiro `app.pode('instrutores','criar')`, linha travada, código conferido no banco, recusa `23503` com as chaves do histórico. A lista de impedimentos foi lida no schema: aula lançada, atribuição (incluindo `turma_disciplina.instrutor_id` e `disciplinas.instrutores_atribuidos`), vínculo, conta, avaliação (incluindo `arquivo_avaliacoes_v1`) e responsável de curso. Nenhuma policy de DELETE e nenhum privilégio de DELETE nasceram. Auditoria: não há tabela onde gravar — `migracao_log` é da migração —, nada criado. Tela: `ExcluirInstrutor.tsx` no rodapé, ao lado de desativar, oculto sem `criar`, desabilitado com motivo, diálogo que exige o código. pgTAP `098` (17 asserções), RLS com sessão real (10 casos: recusa com aula, atribuição, vínculo e conta; perfis sem permissão; código errado; DELETE direto; sucesso) e ponta a ponta (cadastrar, excluir pelo código, sumir do banco; botão desabilitado com motivo). **Conferido por defeito deliberado**: sem a conferência de vínculo, a asserção 8 do `098` reprovou. ⚠️ Medido: **nenhum** dos 177 instrutores da base real é excluível.
- [X] T131 Atualizar `checklists/fechamento.md`, `spec.md`, `data-model.md`, `paridade.md`, `quickstart.md`, a regra 4 e o estado atual do `CLAUDE.md` ✅ Checklist com 20 de 22 passando; CHK020 e CHK021 pendentes por falta de material.
...
| 8.3 — Respostas ao checklist (CHK004, CHK005) | 2 |
