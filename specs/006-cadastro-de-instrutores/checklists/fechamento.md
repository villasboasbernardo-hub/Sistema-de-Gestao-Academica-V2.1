...
- [x] CHK004 O recorte do aviso "Data de início de docência não informada" está decidido — quem está sem a data, ou só quem também está sem capacitação? [Ambiguidade, Spec §FR-017, §FR-027] — ✅ **Decidido por Bernardo em 15/09/2026**: só quem também está sem capacitação registrada, porque quem tem capacitação nunca dispararia o alerta. A leitura virou decisão, anotada no `FR-017` e no contrato. *Registro anterior:* ⚠️ **Pede decisão.** A decisão diz "em lugar do alerta"; a implementação lista só quem também está sem capacitação (148 na base real; seriam 176 cobrando todos). **Correção mecânica feita**: a spec atribuía esse recorte à decisão de Bernardo, e agora ele aparece separado, como *leitura aplicada, a confirmar*.
- [x] CHK005 O período avaliado pelo alerta de faixa está definido — semanas já passadas alertam? a janela de uma atribuição do ano anterior que invade o ano corrente entra? [Gap, Spec §FR-016] — ✅ **Decidido por Bernardo em 15/09/2026**: todas as semanas ISO do ano corrente cobertas por atribuição ativa, passadas inclusive, e a parte do ano corrente de janela vinda do ano anterior. ⚠️ **Mudou o código**: a leitura anterior deixava de fora a janela vinda do ano anterior. *Registro anterior:* ⚠️ **Pede decisão.** A implementação avalia as atribuições do ano corrente pela data de início prevista (T011 c), inclusive semanas passadas. Registrado no contrato como *leitura aplicada, a confirmar*.
...
- [x] CHK008 A divergência do `RN-INST-03` entre o documento 04 (cinco campos) e a spec (quatro) está registrada no lugar onde o projeto lista divergências? [Rastreabilidade, Spec §Regras que pareceram estranhas] — **correção mecânica feita**: entrou como item 7. ⚠️ **Pede decisão**: emendar o documento 04 é decisão à parte, como no item 1 da mesma seção. ✅ **Decidido por Bernardo em 15/09/2026**: o `RN-INST-03` mantém os cinco obrigatórios, com a especialidade delimitada a militar e recusada em cadastro novo (tela e gatilho `20260915140000`) — a spec voltou a bater com o documento 04, que não foi alterado. Não há mais o que emendar nele.

## Clareza

- [x] CHK009 "Mais de um ano" está quantificado, com o caso de fronteira de um ano exato? [Clareza, Spec §FR-017] — estrito; um ano exato não alerta, pelo texto do `RF-INSTR-16`.
...
- [x] CHK019 A spec diz se os alertas aparecem na ficha de instrutor **inativo**? [Gap, Spec §FR-016, §FR-017, §US5] — ⚠️ **Pede decisão** (baixa urgência). Nada está escrito; hoje a ficha do inativo mostra os alertas como a do ativo. ✅ **Decidido por Bernardo em 15/09/2026**: ficha de inativo não exibe alerta normativo nem aviso de cadastro. Implementado na ficha e coberto por caso de ponta a ponta.

## Requisitos parados

- [ ] CHK020 O `FR-026.6` (legenda clicável) tem comportamento descrito o bastante para ser implementado? [Completude, Spec §FR-026.6] — ⚠️ **Pede decisão.** A conferência no código real da v2.0 não achou o comportamento. Pela regra 3 do `CLAUDE.md` (*paridade antes de novidade*), sem origem na v2.0 isto é **novidade**, e precisa ser escrito e autorizado como tal. ⏸️ **Pendência mantida em 15/09/2026, por falta de material**: nada implementado.
- [ ] CHK021 O `FR-031` (ficha A4) tem todos os insumos que o layout oficial exige? [Dependência, Spec §FR-031] — ⚠️ **Pede insumo.** Falta o selo oficial "Marinha do Brasil — Hidrografia e Navegação" (`image2.png` do modelo); o brasão do CIAARA já está no repositório. ⏸️ **Pendência mantida em 15/09/2026, por falta de material**: nada implementado nem improvisado.

## Não funcionais

- [x] CHK022 Existe critério de tempo de resposta para a ficha do instrutor? [Gap, Não funcional] — ⚠️ **Pede decisão.** Nenhum requisito fala em tempo. Medido em 15/09/2026, com a base real: 1,5 a 2,4 s por ficha com um acesso, 4,2 a 5,1 s com quatro simultâneos; `vw_instrutor_carga_anual` custa ~700 ms sob RLS. O ponta a ponta ganhou espera medida e o achado ficou registrado, sem otimização nesta fatia. ✅ **Decidido por Bernardo em 15/09/2026**: **não** se cria requisito de tempo de resposta. A medição fica registrada nas *Assumptions* da spec (item 6) como ponto de partida para quem otimizar depois.
...
- **Atualização de 15/09/2026, com as respostas de Bernardo**: **CHK004 e CHK005 fechados** — 16 passam.
  Seguem abertos CHK008, CHK012, CHK019, CHK020, CHK021 e CHK022.
...
- **Atualização de 15/09/2026, com o restante das respostas**: fechados também CHK008, CHK012, CHK019 e
  CHK022 — **20 passam**. Seguem abertos só **CHK020** e **CHK021**, pendentes por falta de material.
