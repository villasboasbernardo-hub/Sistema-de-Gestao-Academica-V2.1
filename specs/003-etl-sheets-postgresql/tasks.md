...
- [X] T019.1 [US1] Implementar em `_comum.py` os **dois caminhos de data**, separados de propósito: coluna `date` sai e entra como **literal `YYYY-MM-DD`, sem conversão de fuso**; coluna `timestamptz` usa **`America/Sao_Paulo`**. Misturá-los é como o deslocamento de um dia nasce (FR-019, FR-019.1) ✅ `_comum.data_civil()` e `instante()`, e no SQL `T.DATA_CIVIL` sai como `left(…,10)::date` **sem fuso** enquanto `T.INSTANTE` usa `at time zone 'America/Sao_Paulo'`. Os dois caminhos nunca se cruzam.
- [ ] T020 [US1] Escrever a normalização (etapa 2): `bruto/*.csv` → `normalizado/<tabela>.csv`, com as colunas do destino e sufixo `_codigo` onde guarda um `ID_*` a resolver ⚠️ **DESVIO DELIBERADO, registrado em 08/09/2026:** não existe artefato `normalizado/<tabela>.csv`. A normalização deixou de ser um passo com arquivo e passou a ser **expressão SQL gerada a partir do `mapa.py`** na promoção `staging`→`public`. O artefato inspecionável da etapa 2 passou a ser a própria `staging` — que é consultável, comparável e sobrevive à carga (é truncada no INÍCIO da execução seguinte, não descartada no fim). O contrato pipeline pede um CSV; a implementação entrega uma tabela. **Quem for reescrever o contrato precisa decidir qual dos dois fica.**
...
- [X] T021.1 [US1] Implementar a falha nomeada para **coluna obrigatória sem valor na origem** (os cinco `NOT NULL` de `instrutores`, D-08): aborta dizendo qual instrutor e qual coluna. **Valor nunca é fabricado** (FR-018.1) ✅ Coluna obrigatoria sem valor **aborta com nome**: `conferir_antes_de_escrever` (destino inexistente), `conferir_dominios` (valor fora do dominio, com contagem) e `conferir_orfaos` (chave sem par). Nenhum valor e fabricado. ⚠️ Os cinco `NOT NULL` de `instrutores` da D-08 mudaram de natureza: a medicao mostrou que `esp_hab_obs` esta legitimamente vazia em 15 dos 177, e a coluna deixou de ser obrigatoria (migration `20260908084000`).
- [X] T022 [US1] Implementar a rejeição de **valor fora de domínio fechado**: interrompe nomeando tabela, coluna e valor. **Nunca** vira padrão silencioso (FR-017). A verificação por coluna cobre os **três** eixos: formato, faixa e domínio (FR-018.2) ✅ `conferir_dominios` interrompe nomeando tabela, coluna, valor e **contagem de linhas**, antes de escrever. Nunca vira padrao silencioso.
...
- [X] T030 [US1] Implementar a etapa 4 em `executar.py`: promoção na ordem de `ordem.py`, **numa transação única** — ou as 25 tabelas entram, ou nenhuma (FR-005) ✅ `promover.promover()` na ordem de `ordem.py`, transacao unica. **Provado em T041.**
- [X] T031 [US1] Garantir que `config_listas` é a **tabela nº 1**: quatro gatilhos validam contra ela, e carregá-la depois faz os 1.566 registros de aula falharem com "valor fora do domínio", sem a mensagem dizer que o problema é a ordem (contrato pipeline P-5) ✅ `config_listas` e a primeira de `ORDEM_DE_CARGA`, e `semear_vocabulario_usado()` completa o vocabulario **antes** de `registros_aula`. ⚠️ Achado: a lista da v2.0 nunca conteve os valores que a propria v2.0 usava — 10 semeados, marcados por procedencia.
...
- [X] T033 [US1] Tratar a **armadilha B** do documento 30 §3.2: o gatilho anti-escalonamento bloqueia a própria carga de `usuarios` ✅ **Medido, nao presumido**: `usuarios` promoveu as 3 linhas sem que o gatilho anti-escalonamento disparasse — ele compara `OLD`/`NEW` e no `INSERT` nao ha `OLD`.
- [X] T034 [P] [US1] Escrever os módulos de tabela 1 a 8 em `scripts/etl/tabelas/`: `config_listas`, `config_parametros`, `perfil_permissao`, `cursos`, `configuracoes_horario`, `horarios_tempos_aula`, `curso_regime_historico`, `turmas` ✅ **25 de 25 transcritas** em `scripts/etl/mapa.py`, do documento 31 §1 a §25. **382 colunas**, 343 migráveis e 39 declaradas como descartadas/geradas/staging — **declaradas, nunca omitidas**. `conferir_cobertura()` fecha nos dois sentidos: nada na ordem sem mapa, nada mapeado fora da ordem.
- [X] T035 [P] [US1] Escrever os módulos 9 a 12: `instrutores` (⚠️ os cinco `NOT NULL` da pendência D-08 podem falhar — §6.4), `disciplinas`, `turma_disciplina` (**210 linhas**), `instrutor_disciplina` ✅ Incluídas no mapa: `instrutores` (com os cinco `NOT NULL` da D-08 marcados), `disciplinas`, `turma_disciplina` (210 linhas, com `instrutor_id` e `ch_prevista_por_instrutor` agora existentes pela P-6) e `instrutor_disciplina`.
- [X] T036 [P] [US1] Escrever os módulos 13 a 16: `usuarios` (⚠️ `USR-04` é linha-fantasma), `usuario_curso`, `responsaveis_curso`, `avaliacoes_planejadas` ✅ Incluídas: `usuarios` (com a armadilha B e a linha-fantasma `USR-04` anotadas), `usuario_curso`, `responsaveis_curso` (com o literal `GERAL` → `NULL`, D-04) e `avaliacoes_planejadas`.
- [ ] T037 [US1] Escrever o módulo de `registros_aula` (**1.566 linhas**) — consome `staging.ue_cruzamento` para preencher `unidade_ensino_id`, e deixa nulo onde o veredito não for `casado` ⚠️ **ABERTO, e é o único item do Épico 2 que ficou.** `registros_aula` carrega as 1.566 linhas, mas `unidade_ensino_id` sai **nula em todas** e `unidades_ensino` está **vazia**. O cruzamento existe e tem **901 `casado`** (`dados/normalizado/ue_cruzamento.csv`); falta reconciliar um TERCEIRO espaço de nomes — a sigla de curso do catálogo da DEnsM não bate com `cursos.codigo` em **8 dos 21**, e o `disciplina_id` fecha em **84 de 134** pelo nome. Ratificado por Bernardo em 08/09: nulo é o retrato fiel enquanto o cruzamento não for aplicado.
- [X] T038 [P] [US1] Escrever os módulos 18 a 24: `avaliacoes`, `arquivo_avaliacoes_v1`, `atividades_nao_letivas` (**664**), `feriados`, `janelas_curso`, `reservas_proens`, `planejamento_anual` (nasce **vazia**) ✅ Incluídas: `avaliacoes`, `arquivo_avaliacoes_v1`, `atividades_nao_letivas`, `feriados`, `janelas_curso`, `reservas_proens` e `planejamento_anual` (que nasce vazia, e isso é correto).
- [X] T039 [US1] Tratar a regra de não sobreposição de vigência em `curso_regime_historico`, que **pode recusar dado legítimo** vindo da planilha (documento 30 §6.5) ✅ **Medido**: as 29 linhas de `curso_regime_historico` entraram sem que o `EXCLUDE` de vigencia recusasse nenhuma. ⚠️ O que recusou foram outros dois CHECKs, e por outro motivo: `regime_tempos_valido` e `regime_duracao_normativa` nao previam o curso EAD, que traz `0` (migration `20260908085000`).
- [X] T040 [US1] Escrever `scripts/etl/executar.py` como **ponto de entrada único**, com `--ambiente`, `--primeira-carga` e `--somente-reconciliar`. Não existe rodar uma tabela isolada (contrato pipeline P-7) ✅ `scripts/etl/executar.py`, com `--ambiente`, `--primeira-carga`, `--somente-reconciliar` e `--conexao`. **O codigo de saida e o veredito da reconciliacao**: 0 aprovada, 1 bloqueada, 2 nao conferida, 3 abortou antes de escrever. `--ambiente` diferente de `local` com a conexao local padrao e **recusado**. Ciclo completo do zero: `db:reset` -> pipeline -> APROVADA, saida 0.
- [X] T041 [US1] Provar a atomicidade: injetar defeito numa tabela do meio da ordem e confirmar **zero linha gravada** em qualquer tabela, com mensagem nomeando tabela e linha (quickstart V-2) ✅ **PROVADO** (`scripts/etl/provar_carregador.py`): defeito injetado na **9ª tabela de 25** — oito ja inseridas dentro da transacao — e o resultado foi **zero linha gravada** nas 23 tabelas de carga, com as 2 semeadas pela migration intactas, corretamente.
- [X] T042 [US1] Provar a idempotência: duas execuções do zero produzem contagens e checksums idênticos **nas tabelas de negócio**, com o checksum **excluindo `id` e o quarteto de auditoria** e com `migracao_log` **fora da comparação** — os cinco variam por construção (FR-006, FR-006.1, quickstart V-5) ✅ **PROVADO, e o defeito era do proprio criterio.** Duas cargas do zero davam **13 checksums diferentes de 23**: o FR-006.1 manda excluir `id` e o quarteto, mas **nao as colunas de FK** — `turmas.curso_id` guarda o `gen_random_uuid()` de `cursos`, outro a cada carga. Corrigido trocando a FK pelo `codigo` do alvo (estavel E sensivel a para onde a linha aponta); mais dois achados: `horarios_tempos_aula` nao tem `codigo` e caia numa ordenacao nao deterministica, e `arquivado_em` e o `now()` da carga. **23 de 23 coincidem.**
...
- [X] T043 [US2] Implementar **R-01** em `scripts/etl/reconciliar.py`: contagem por tabela contra o documento 05 §10. **Uma linha de diferença bloqueia** (FR-009) ✅ `r01_contagem` conferindo **pela procedência** (`origem_migracao_v1 like '<aba>:%'`), nao pelo total: `config_listas` tem 96 linhas legitimamente — 72 da v2.0, 14 semeadas pela migration do Epico 1 e 10 pelo proprio ETL. Comparar totais acusaria 24 divergencias inexistentes. O unico descarte aceito esta declarado em `DESCARTES_DECLARADOS`.
- [X] T043.1 [P] [US2] Tratar **contagem esperada zero** como valor válido e verificado — hoje `planejamento_anual`. Tabela **ausente** é falha distinta e reportada como tal (FR-009.3) ✅ `planejamento_anual` com 0 linhas e verificada e aprovada — 0 esperado e 0 obtido fecham. Tabela **ausente** e falha distinta: `conferir_antes_de_escrever` acusa 'tabela de destino inexistente no banco'.
- [X] T044 [US2] Implementar **R-02**, o somatório de **TA por turma**, origem × destino, incluindo aula, aplicação de avaliação, vista de prova e atividade não letiva. Aceite: **0 nas 29 turmas, sem tolerância** (FR-010) ✅ `r02_tempos_por_turma`, com aula + avaliacao + vista de prova + atividade nao letiva. **Zero divergencia nas 28 turmas.**
- [X] T044.1 [US2] Somar o lado da **origem em bruto**: `staging.*` **sem cláusula `WHERE`**, com a conversão numérica feita **na própria consulta de reconciliação** — nunca reaproveitando a conversão da normalização, que é o que está sob suspeita (FR-014.1, plan.md §A nota que o FR-014.1 exigiu) ✅ O lado da origem sai de `staging.*` sem `WHERE`, com a conversao numerica escrita na propria consulta de reconciliacao.
- [X] T045 [P] [US2] Implementar **R-03**: zero FK órfã em toda a base (FR-011) ✅ `r03_orfas`, sobre **todas** as FKs do schema. ⚠️ A primeira versao usava `information_schema` juntando por `constraint_name` sem parear as colunas — produto cartesiano, que acusou 188 orfas inexistentes comparando `avaliacoes.curso_id` com `disciplinas.id`. Refeita com `pg_constraint` e `conkey[1]`/`confkey[1]`.
- [X] T045.1 [US2] Fazer a verificação de órfãs ser **por coluna**, com a lista das FK anuláveis declarada — `registros_aula.unidade_ensino_id`, `registros_aula.instrutor_id`, `turma_disciplina.instrutor_id`. **Nulo não é órfã**: sem isso, os 1.566 nulos legítimos seriam contados como violação (FR-011.1) ✅ Por coluna, e **nulo nao e orfa**: a verificacao so olha valor preenchido que aponta para linha inexistente.
- [X] T046 [P] [US2] Implementar **R-04** como **relação estrutural**, não como literal: as três identidades fecham sobre a linha de base vigente. Os números de 02/08 são a foto inicial (FR-012, redação corrigida) ✅ `r04_identidades` como relacao estrutural — origem x destino e soma das categorias — nunca contra os literais de 02/08.
- [X] T047 [P] [US2] Implementar **R-05**: `codigo` não nulo e único, procedência preenchida, em 100% das linhas (FR-002) ✅ `r05_identidade_e_procedencia`. `SEMEADAS_PELO_SCHEMA` declara as duas tabelas que o proprio schema popula; em qualquer outra, linha sem procedencia bloqueia.
- [X] T048 [P] [US2] Implementar **R-07**: `turma_disciplina` com **89** períodos herdados e **121** em branco, exatamente como na origem (FR-013) ✅ `r07_periodos`: **89 herdados e 121 em branco**, exatos.
- [X] T049 [US2] Implementar o relatório em `dados/relatorio_divergencia.md` com **veredito explícito** e **cada divergência nomeada** — "2 divergências" não serve; serve "tabela X, linha Y, esperado Z, obtido W" (FR-014, contrato C-2) ✅ `dados/relatorio_divergencia.md`, com veredito explicito e cada divergencia no formato `verificacao · tabela · linha · esperado · obtido`.
- [X] T049.1 [US2] Fazer a reconciliação **falhar com erro nomeado** quando não conseguir ler — staging ausente, conexão perdida. Ausência de divergência por ausência de leitura **não é aprovação** (FR-014.2, `RN-DEG-01`) ✅ `NaoFoiPossivelReconciliar` — staging vazia, conexao perdida ou consulta que falha levantam erro nomeado e saem com codigo 2. Nao ler nunca vira 'nenhuma divergencia'. A sessao ainda e aberta `read_only`, impondo o contrato C-4 pelo banco.
- [X] T049.2 [US2] Implementar o **checksum canônico** em `reconciliar.py`: **`md5()`** sobre a concatenação textual dos valores, ordenada pelas colunas de negócio, **excluindo `id` e o quarteto de auditoria**, e **pulando `migracao_log`** por lista explícita (FR-006, FR-006.1) ✅ `r08_checksums`: `md5()` sobre as colunas de negocio ordenadas pelo nome, linhas ordenadas por `codigo`, sem `id` nem o quarteto de auditoria; `migracao_log` fora por `ordem.FORA_DA_IDEMPOTENCIA`.
- [X] T050 [US2] Separar no relatório o que **bloqueia** (R-01 a R-08) do que **informa** (U-01, U-02) e do que é **esperado** (U-03, os 17 cursos sem fonte de UE) — contrato reconciliacao C-5 ✅ Tres secoes no relatorio: bloqueia / informa / esperado.
- [X] T051 [US2] Escrever as invariantes em `supabase/tests/090_reconciliacao_etl.sql` (pgTAP), uma asserção **nomeada** por verificação bloqueante. A lista é **fechada** em R-01 a R-08 — critério de bloqueio não se acrescenta em tempo de execução (FR-015.1) ✅ `supabase/tests/090_reconciliacao_etl.sql`, 8 asseracoes nomeadas. **Pulam com `skip` motivado quando a base esta vazia** — `pnpm db:reset` recria o schema sem dado, e afrouxar a asserção ate passar nos dois estados a esvaziaria. R-06 e R-08 nao pulam: sao estruturais.
- [X] T052 [US2] **Provar que a R-02 prova algo**: mover um registro de aula de turma à mão e confirmar que a reconciliação **acusa**, com a contagem total inalterada. Se passar, a verificação não vale nada (quickstart V-3, SC-008) ✅ **PROVADO** (`scripts/etl/provar_r02.py`): `REG-0176` movido de `C-Ap-HN 2026` para `EST-QF-MAREFLU 2026`, contagem total intacta em 1.566, e a R-02 acusou os dois lados (−3 e +3 tempos). Restaurado, volta a APROVADA. ⚠️ Achado de brinde: a primeira tentativa foi **recusada pela catraca** `reg_aula_ue_so_nula_no_historico` — editar linha migrada sem UE nao passa, exatamente como desenhado.
...
- [X] T053 [US3] Preencher `codigo` com o `ID_*` da v2.0 **verbatim** e `origem_migracao_v1` em toda linha migrada (FR-002) ✅ `codigo` recebe o `ID_*` verbatim e `origem_migracao_v1` sai como `<aba>:<chave>` em toda linha migrada. **R-05 prova nas 26 tabelas.**
...
- [X] T058 [US3] Provar que `migracao_log` recusa `UPDATE` e `DELETE` **de todo perfil, inclusive o administrativo** — o gatilho já existe desde o Épico 1; aqui se prova que a carga não o contorna (SC-006, quickstart V-6) ✅ **PROVADO** (`provar_carregador.py`): `UPDATE` e `DELETE` em `migracao_log` recusados **conectado como `postgres`** — dono do schema, acima de `service_role`. Testar com perfil comum nao provaria nada: a RLS o barraria antes e o gatilho ficaria sem teste.
...
- [X] T060 [US4] Garantir que **nenhuma etapa escreve na origem** — leitura apenas, em todo o pipeline (FR-021) ✅ Nenhuma etapa escreve na origem: a extracao abre o `.xlsx` em leitura, e a reconciliacao abre a **sessao `read_only`** — o contrato C-4 passa a ser imposto pelo banco, nao pela boa vontade do modulo.
...
- [X] T073 Atualizar o *Estado atual* do `CLAUDE.md`: Épico 2 e o que o Épico 3 herda — inclusive `NEXT_PUBLIC_URL_APLICACAO`, deixada fora do Épico 0, e a Server Action de convite, primeiro consumidor real de `lib/supabase/admin.ts` ✅ Épico 2 marcado como **CONCLUÍDO**, com o que ficou aberto (carga das 572 UEs) registrado em linha própria; Q1.b e a pendência de hospedagem marcadas como fechadas; os nove achados do Épico 2 escritos na seção que o agente lê a cada sessão.
