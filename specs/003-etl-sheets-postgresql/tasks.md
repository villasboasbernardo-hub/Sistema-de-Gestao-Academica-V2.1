...
- [ ] T034 [P] [US1] Escrever os módulos de tabela 1 a 8 em `scripts/etl/tabelas/`: `config_listas`, `config_parametros`, `perfil_permissao`, `cursos`, `configuracoes_horario`, `horarios_tempos_aula`, `curso_regime_historico`, `turmas` 🟨 **EM ANDAMENTO.** A forma mudou: em vez de 25 módulos com `for` e `insert` escritos à mão — 25 oportunidades de divergir em silêncio —, o de-para vive em `scripts/etl/mapa.py` como **registro declarativo**, e a política de promoção fica em `_comum.py`. É o que o documento 30 §2.6 pede em texto: *"a mesma política nas 25 tabelas, e não 25 variações parecidas"*. **3 de 25 transcritas** (`config_listas`, `cursos`, `turmas`); as 22 restantes estão em `mapa.PENDENTES`, contáveis por código.
